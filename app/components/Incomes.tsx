...
const Incomes = ({transactions, setTransactions} : propTypes) => {
    ...
    return (
        <div className="flex flex-col items-center max-w-fit mx-2.5">
            ...
            {incomes.length > 0 ? incomes.map((income, index: number) => (
                <TransactionsCard key={index} id={income.id} title={income.title} note={income.note} amount={Number(income.amount)} type={"INCOME"} setTransactions={setTransactions} />
            )):
