...
const Expenses = ({ transactions, setTransactions }: propTypes) => {
    ...
    return (
        <div className="flex flex-col items-center max-w-fit  mx-2.5">
            ...
            {expenses.length > 0 ? expenses.map((expense, index: number) => (
                <TransactionsCard key={index} title={expense.title} note={expense.note} amount={Number(expense.amount)} id={expense.id} type={"EXPENSE"} setTransactions={setTransactions}/>
